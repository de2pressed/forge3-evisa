      ...
      <header className="app-header">
        <div className="container app-nav">
          <Link className="brand" href="/">
            <svg className="brand-mark" viewBox="0 0 32 32" fill="none"><path d="M16 2.5 19.4 12l9.1 4-9.1 4L16 29.5 12.6 20 3.5 16l9.1-4L16 2.5Z" fill="currentColor"/></svg>
            <span>Forge<small>India eVisa</small></span>
          </Link>
          <span className="secure-label">◈ Secure application · Progress saved on this device</span>
      ...
      <main className="app-main" id="applicationFlow">
        <div className="container">
          <div className="app-title">
            <div><div className="eyebrow">Normal e&#x2011;Business Visa</div><h1 className="serif">Let&#39;s get you to India.</h1></div>
            <p>You&#39;ll need your passport, a recent photograph and business evidence. Most people finish in about 10 minutes.</p>
          </div>

          <div className="progress" aria-label="Application progress">
            {[{ n: 1, label: 'Applicant' }, { n: 2, label: 'Business' }, { n: 3, label: 'Documents' }, { n: 4, label: 'Payment' }, { n: 5, label: 'Review' }].map(({ n, label }) => (
          ...
          <div className="app-layout">
            <section className="form-panel card">
              ...
              <div className={`form-step${currentStep === 1 ? ' active' : ''}`} data-step="1">
                <h2>About you and your trip</h2>
                <p className="form-intro">Enter details exactly as they appear in the passport you will travel with.</p>
                <form onSubmit={(e) => { e.preventDefault(); setStep(2); }}>
                  ...
                  <div className="form-actions">
                    ...
                    <button className="btn btn-primary" type="submit">Continue to business details →</button>
                  </div>
                </form>
              </div>
              ...
              <div className={`form-step${currentStep === 2 ? ' active' : ''}`} data-step="2">
                <h2>Your business visit</h2>
                <p className="form-intro">Tell us why you&#39;re travelling and who you&#39;ll meet in India.</p>
                <form onSubmit={(e) => { e.preventDefault(); setStep(3); }}>
                  <div className="form-grid">
                    <div className="field full"><label htmlFor="activity">Primary business activity</label><select id="activity"><option>Business meetings or negotiations</option><option>Supplier or partner visit</option><option>Trade or sales activity</option><option>Recruitment or internal meetings</option><option>Technical consultation</option></select></div>
                    <div className="field"><label htmlFor="employer">Your employer or organisation</label><input id="employer" required placeholder="Northstar Labs Ltd" /></div>
                    <div className="field"><label htmlFor="role">Your job title</label><input id="role" required placeholder="Product Director" /></div>
                    <div className="field"><label htmlFor="indiaCompany">Indian company or organisation</label><input id="indiaCompany" required placeholder="Aster Technologies Pvt Ltd" /></div>
                    <div className="field"><label htmlFor="indiaContact">Contact person in India</label><input id="indiaContact" required placeholder="Priya Mehta" /></div>
                    <div className="field full"><label htmlFor="indiaAddress">Indian organisation address</label><textarea id="indiaAddress" required placeholder="Full office address in India"></textarea></div>
              ...
              <div className={`form-step${currentStep === 3 ? ' active' : ''}`} data-step="3">
                ...
                <p className="form-intro">Use clear, current files. We&#39;ll show the status of each one before you continue.</p>
                <div className="upload-list">
                  {[{ key: 'passport', icon: '▣', label: 'Passport bio page' }, { key: 'photo', icon: '◉', label: 'Recent photograph' }, { key: 'business', icon: '▤', label: 'Business card or invitation' }].map(({ key, icon, label }) => (
              ...
              <div className={`form-step${currentStep === 4 ? ' active' : ''}`} data-step="4">
                ...
                <p className="form-intro">The applicable eVisa fee depends on passport nationality. The full amount is shown before payment.</p>
                <div className="fee-box">
                  <div className="eyebrow">United Kingdom passport</div>
                  <div className="fee-row"><span>Normal e&#x2011;Business Visa fee</span><strong>Calculated for nationality</strong></div>
              ...
              <div className={`form-step${currentStep === 5 ? ' active' : ''}`} data-step="5">
                <div className={submitted ? 'hidden' : ''} id="reviewContent">
                  ...
                  <dl className="review-list">
                    ...
                    <div className="review-row"><dt>Trip</dt><dd>Business meetings · Arrival 10 August 2026</dd><button type="button" onClick={() => setStep(1)}>Edit</button></div>
                    <div className="review-row"><dt>Business contact</dt><dd>Aster Technologies Pvt Ltd · Bengaluru</dd><button type="button" onClick={() => setStep(2)}>Edit</button></div>
                    <div className="review-row"><dt>Documents</dt><dd>Passport, photo and business evidence ready</dd><button type="button" onClick={() => setStep(3)}>Edit</button></div>
            ...
            <aside>
              <div className="summary card">
                ...
                <h3>Normal e&#x2011;Business</h3>
                <div className="summary-row"><span>Validity</span><strong>365 days</strong></div>
                <div className="summary-row"><span>Entries</span><strong>Multiple</strong></div>
                <div className="summary-row"><span>Stay</span><strong>Up to 180 days per visit</strong></div>
                ...
                <div className="summary-note"><strong>Normal business activity only.</strong><br />Conference, sports, film and other specialised activities may require different evidence or routes.</div>
              </div>
            </aside>
          </div>
        </div>
